...
reflector --verbose -a 48 --latest 20 --sort rate --save /etc/pacman.d/mirrorlist
if [ ! -d "/mnt" ]; then
    mkdir /mnt
fi
echo -ne "
...
if [[ ! -d "/sys/firmware/efi" ]]; then
    ...
    pacstrap /mnt base base-devel linux linux-firmware linux-headers efibootmgr --noconfirm --needed
fi
...
echo -ne "
-------------------------------------------------------------------------
                    Setup Language to HU and set locale
-------------------------------------------------------------------------
"
...
echo -e "Install and setting os-prober"
